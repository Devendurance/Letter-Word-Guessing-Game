...
var ARC_PRIVATE_MAINNET_HEADER = "X-ARC-PRIVATE-MAINNET-ENABLED";
function arcPrivateMainnetHeaders(arcPrivateMainnet) {
  if (!arcPrivateMainnet) {
    return {};
  }
  return { [ARC_PRIVATE_MAINNET_HEADER]: "true" };
}
...
var BatchFacilitatorClient = class {
  ...
  beforeVerifyHooks = [];
  afterVerifyHooks = [];
  onVerifyFailureHooks = [];
  beforeSettleHooks = [];
  afterSettleHooks = [];
  onSettleFailureHooks = [];
  /**
  ...
  constructor(config = {}) {
    ...
    this._arcPrivateMainnet = config.arcPrivateMainnet ?? false;
  }
  gatewayApiHeaders(base2 = {}) {
    return { ...base2, ...arcPrivateMainnetHeaders(this._arcPrivateMainnet) };
  }
  /**
   * Register a hook that runs before facilitator verification.
   * Return `{ abort: true, reason }` to short-circuit and treat the payment
   * as invalid (returns `{ isValid: false, invalidReason: reason }`).
   */
  onBeforeVerify(hook) {
  ...
  /**
   * Register a hook that runs after a successful verification (`isValid: true`).
   * Not invoked when verification fails — use {@link onVerifyFailure} for that.
   */
  onAfterVerify(hook) {
  ...
  /**
   * Register a hook that runs when verification fails — either when the
   * facilitator returns `isValid: false` or when an exception is thrown.
   * Return `{ recovered: true, result }` to override the failure with a
  ...
  /**
   * Register a hook that runs before facilitator settlement. Return
   * `{ abort: true, reason }` to throw `Error("Settlement aborted: <reason>")`.
   */
  onBeforeSettle(hook) {
  ...
  /**
   * Register a hook that runs after a successful settlement.
   */
  onAfterSettle(hook) {
  ...
  /**
   * Register a hook that runs when settlement throws. Return
   * `{ recovered: true, result }` to swallow the error and return `result`
   * to the caller as if settlement had succeeded.
   */
  ...
  async verify(paymentPayload, paymentRequirements) {
    ...
    try {
      const verifyResult = await this.callVerify(paymentPayload, paymentRequirements);
      if (!verifyResult.isValid) {
        ...
        for (const hook of this.onVerifyFailureHooks) {
          ...
          if (result && "recovered" in result && result.recovered) {
            ...
            for (const [index, afterHook] of this.afterVerifyHooks.entries()) {
              try {
      ...
      for (const [index, hook] of this.afterVerifyHooks.entries()) {
        try {
  ...
  async settle(paymentPayload, paymentRequirements) {
    ...
    try {
      const settleResult = await this.callSettle(paymentPayload, paymentRequirements);
      const resultContext = {
        ...context,
        result: settleResult
      };
      for (const [index, hook] of this.afterSettleHooks.entries()) {
        try {
          await hook(resultContext);
        } catch (err) {
          warnHookFailure(
  ...
  /**
   * HTTP call into Circle Gateway's /verify endpoint. Split out so the hook
   * machinery in `verify()` reads cleanly.
   */
  async callVerify(paymentPayload, paymentRequirements) {
    let headers = this.gatewayApiHeaders({
  ...
  /**
   * HTTP call into Circle Gateway's /settle endpoint. Split out so the hook
   * machinery in `settle()` reads cleanly.
   */
  async callSettle(paymentPayload, paymentRequirements) {
    let headers = this.gatewayApiHeaders({
      "Content-Type": "application/json"
    });
    if (this._createAuthHeaders) {
...
// src/client/GatewayClient.ts
var arcMainnet = chains.arc ?? defineChain({
  id: 5042,
  name: "Arc",
...
var arcTestnetChain = chains.arcTestnet ?? defineChain({
  id: 5042002,
  name: "Arc Testnet",
  nativeCurrency: { decimals: 18, name: "USDC", symbol: "USDC" },
  rpcUrls: {
...
function buildProtectedRequestContext(req) {
  const url = req.url ?? "/";
...
function createGatewayMiddleware(config) {
  const facilitator = new BatchFacilitatorClient({
    ...
    arcPrivateMainnet: config.arcPrivateMainnet
  });
  ...
  const protectedRequestHooks = [];
  const beforeVerifyHooks = [];
  const afterVerifyHooks = [];
  const onVerifyFailureHooks = [];
  const beforeSettleHooks = [];
  const afterSettleHooks = [];
  const onSettleFailureHooks = [];
  const onVerifiedPaymentCanceledHooks = [];
  async function runAfterVerifyHooks(context, verifyResult) {
    const resultContext = { ...context, result: verifyResult };
    let skipHandler;
    for (const [index, hook] of afterVerifyHooks.entries()) {
      try {
        ...
        warnHookFailure("GatewayMiddleware", "afterVerify", `manual hook #${index}`, err);
      }
    }
    return skipHandler;
  }
  async function runVerifyLifecycle(paymentPayload, requirements) {
    const context = {
      paymentPayload,
      requirements
    };
    for (const [index, hook] of beforeVerifyHooks.entries()) {
      try {
        const result = await hook(context);
        if (result && "abort" in result && result.abort) {
        ...
        if (result && "skip" in result && result.skip) {
          const skipHandler = await runAfterVerifyHooks(context, result.result);
          return { kind: "verified", result: result.result, skipHandler };
        }
      } catch (err) {
        warnHookFailure(
          "GatewayMiddleware",
    ...
    try {
      ...
      if (!verifyResult.isValid) {
        ...
        for (const hook of onVerifyFailureHooks) {
          try {
            const recovery = await hook(failureContext);
            if (recovery && "recovered" in recovery && recovery.recovered) {
              const skipHandler2 = await runAfterVerifyHooks(context, recovery.result);
              return { kind: "verified", result: recovery.result, skipHandler: skipHandler2 };
            }
          } catch (err) {
            warnHookFailure("GatewayMiddleware", "onVerifyFailure", "manual hook", err);
          }
        }
        return { kind: "failed", result: verifyResult };
      }
      const skipHandler = await runAfterVerifyHooks(context, verifyResult);
      return { kind: "verified", result: verifyResult, skipHandler };
    } catch (error) {
      const failureContext = {
        ...context,
        error
      ...
      for (const hook of onVerifyFailureHooks) {
        try {
          const recovery = await hook(failureContext);
          if (recovery && "recovered" in recovery && recovery.recovered) {
            const skipHandler = await runAfterVerifyHooks(context, recovery.result);
            return { kind: "verified", result: recovery.result, skipHandler };
          }
        } catch (err) {
          warnHookFailure("GatewayMiddleware", "onVerifyFailure", "manual hook", err);
        }
  ...
  async function runSettleLifecycle(paymentPayload, requirements) {
    const context = {
      paymentPayload,
      requirements
    };
    for (const [index, hook] of beforeSettleHooks.entries()) {
      try {
        const result = await hook(context);
        if (result && "abort" in result && result.abort) {
          return { kind: "aborted", reason: result.reason, message: result.message };
        ...
        if (result && "skip" in result && result.skip) {
          ...
          for (const [idx, after] of afterSettleHooks.entries()) {
            try {
              await after(skipResultContext);
            } catch (err) {
              warnHookFailure(
                "GatewayMiddleware",
                "afterSettle",
          ...
          return { kind: "settled", result: result.result };
        }
      } catch (err) {
        warnHookFailure(
          "GatewayMiddleware",
    ...
    try {
      ...
      const resultContext = { ...context, result: settleResult };
      for (const [index, hook] of afterSettleHooks.entries()) {
        try {
          await hook(resultContext);
        } catch (err) {
          warnHookFailure(
      ...
      return { kind: "settled", result: settleResult };
    } catch (error) {
      const failureContext = {
        ...context,
        error
      };
      for (const hook of onSettleFailureHooks) {
        try {
          const recovery = await hook(failureContext);
          if (recovery && "recovered" in recovery && recovery.recovered) {
            return { kind: "settled", result: recovery.result };
          }
        } catch (err) {
          warnHookFailure("GatewayMiddleware", "onSettleFailure", "manual hook", err);
        }
      }
      return { kind: "thrown", error };
    }
  }
  async function dispatchVerifiedPaymentCanceled(paymentPayload, requirements, options) {
    const context = {
      paymentPayload,
      requirements,
    ...
    for (const [index, hook] of onVerifiedPaymentCanceledHooks.entries()) {
      try {
        await hook(context);
      } catch (err) {
        warnHookFailure(
  ...
  const api = {
    require: (price) => {
      const routeConfig = { price };
      return async (req, res, next) => {
        try {
          const requestContext = buildProtectedRequestContext(req);
          for (const [index, hook] of protectedRequestHooks.entries()) {
            try {
              const result = await hook(requestContext, routeConfig);
          ...
          const verify = await runVerifyLifecycle(paymentPayload, requirements);
          if (verify.kind === "aborted") {
            res.statusCode = 402;
            res.setHeader("Content-Type", "application/json");
            res.end(
              JSON.stringify({
                error: "Payment verification aborted",
                reason: verify.reason,
                ...verify.message ? { message: verify.message } : {}
              })
            );
            return;
          }
          if (verify.kind === "failed") {
            ...
            res.end(
              JSON.stringify({
                ...
                reason: verify.result.invalidReason
              })
            );
            return;
          }
          if (verify.kind === "thrown") {
            throw verify.error;
          }
          const verifyResult = verify.result;
          const skipHandlerDirective = verify.skipHandler;
          const settle = await runSettleLifecycle(paymentPayload, requirements);
          if (settle.kind === "aborted") {
            await dispatchVerifiedPaymentCanceled(paymentPayload, requirements, {
            ...
            res.end(
              JSON.stringify({
                error: "Payment settlement aborted",
                reason: settle.reason,
                ...settle.message ? { message: settle.message } : {}
              })
            );
            return;
          }
          if (settle.kind === "thrown") {
            await dispatchVerifiedPaymentCanceled(paymentPayload, requirements, {
              reason: "handler_failed",
              error: settle.error,
              responseStatus: 402
          ...
          const settleResult = settle.result;
          ...
          if (skipHandlerDirective) {
            const contentType = skipHandlerDirective.contentType ?? "application/json";
            const body = skipHandlerDirective.body ?? {};
            res.statusCode = 200;
    ...
    onProtectedRequest: (hook) => {
      protectedRequestHooks.push(hook);
      return api;
    },
    onBeforeVerify: (hook) => {
      beforeVerifyHooks.push(hook);
      return api;
    },
    onAfterVerify: (hook) => {
      afterVerifyHooks.push(hook);
      return api;
    },
    onVerifyFailure: (hook) => {
      onVerifyFailureHooks.push(hook);
      return api;
    },
    onBeforeSettle: (hook) => {
      beforeSettleHooks.push(hook);
      return api;
    },
    onAfterSettle: (hook) => {
      afterSettleHooks.push(hook);
      return api;
    },
    onSettleFailure: (hook) => {
      onSettleFailureHooks.push(hook);
      return api;
    },
    onVerifiedPaymentCanceled: (hook) => {
      onVerifiedPaymentCanceledHooks.push(hook);
      return api;
    }
  };
  return api;
