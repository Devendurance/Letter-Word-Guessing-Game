...
__export(client_exports, {
  ARC_PRIVATE_MAINNET_HEADER: () => ARC_PRIVATE_MAINNET_HEADER,
  ...
  arcPrivateMainnetHeaders: () => arcPrivateMainnetHeaders,
...
var BatchEvmScheme = class {
  ...
  async createPaymentPayload(x402Version, paymentRequirements) {
    const context = {
      paymentRequired: this.synthesizePaymentRequired(x402Version, paymentRequirements),
   ...
   */
  synthesizePaymentRequired(x402Version, requirements) {
    return {
      x402Version,
...
var arcMainnet = chains.arc ?? (0, import_viem2.defineChain)({
  id: 5042,
  name: "Arc",
...
var arcTestnetChain = chains.arcTestnet ?? (0, import_viem2.defineChain)({
  id: 5042002,
  name: "Arc Testnet",
  nativeCurrency: { decimals: 18, name: "USDC", symbol: "USDC" },
  rpcUrls: {
...
var GatewayClient = class {
  ...
  _arcPrivateMainnet;
  /**
   * The scheme that builds and signs the batched payment payload. `pay()`
   * delegates to this so the buyer flow runs the same lifecycle hooks as the
   * standalone `BatchEvmScheme` / `x402Client` paths.
   */
  batchScheme;
  /**
  ...
  constructor(config) {
    ...
    this._arcPrivateMainnet = config.arcPrivateMainnet ?? config.chain === "arc";
    this.account = (0, import_accounts.privateKeyToAccount)(config.privateKey);
    const rpcUrl = config.rpcUrl ?? chainConfig.rpcUrl;
    if (config.chain === "arc" && !rpcUrl) {
      throw new Error(
        "Arc mainnet has no public RPC yet. Pass a private RPC URL via `new GatewayClient({ chain: 'arc', rpcUrl: 'https://<your-private-arc-rpc>' })` (Alchemy / Quicknode / Circle-provided)."
      );
    }
    ...
    const signer = {
      ...
      signTypedData: (params) => this.account.signTypedData(params)
    };
    this.batchScheme = new BatchEvmScheme(signer);
  }
  // ============================================================================
  // PAYMENT LIFECYCLE HOOKS
  // ============================================================================
  //
  // These delegate to the underlying BatchEvmScheme so that the `pay()` buyer
  // flow runs the same lifecycle hooks as the standalone scheme / x402Client
  // paths — see https://docs.x402.org/advanced-concepts/lifecycle-hooks.
  /**
   * Register a hook that runs before the payment payload is created. Returning
   * `{ abort: true, reason }` causes `pay()` to throw before signing.
   */
  onBeforePaymentCreation(hook) {
    this.batchScheme.onBeforePaymentCreation(hook);
    return this;
  }
  /**
   * Register a hook that runs after the payment payload is successfully created.
   */
  onAfterPaymentCreation(hook) {
    this.batchScheme.onAfterPaymentCreation(hook);
    return this;
  }
  /**
   * Register a hook that runs when payload creation throws. Return
   * `{ recovered: true, payload }` to recover with the provided payload.
   */
  onPaymentCreationFailure(hook) {
    this.batchScheme.onPaymentCreationFailure(hook);
    return this;
  }
  /**
   * Register a hook that runs after the buyer's paid HTTP request completes.
   * Return `{ recovered: true }` to have `pay()` retry once with a fresh payload.
   */
  onPaymentResponse(hook) {
    this.batchScheme.onPaymentResponse(hook);
    return this;
  }
  /**
  ...
  async pay(url, options) {
    ...
    for (let attempt = 0; ; attempt++) {
      const paymentPayload = await this.batchScheme.createPaymentPayload(
        x402Version,
        batchingOption
      );
      ...
      const responseContext = {
        ...
        requirements: batchingOption,
        paymentRequired,
        settleResponse,
        error: paidResponse.ok ? void 0 : new Error(`Payment failed with status ${paidResponse.status}`)
      };
      const recovery = await this.batchScheme.dispatchPaymentResponse(responseContext);
  ...
  async getTransferById(id) {
    ...
    const response = await fetch(
      ...
      { method: "GET", headers: this.gatewayApiHeaders() }
    );
    if (!response.ok) {
  ...
  async searchTransfers(params = {}) {
    ...
    const response = await fetch(url, {
      method: "GET",
      headers: this.gatewayApiHeaders()
    });
    if (!response.ok) {
      const errorText = await response.text().catch(() => response.statusText);
      throw new Error(
  ...
  // ============================================================================
  /**
   * Gateway API headers (Arc mainnet internal preview).
   */
  gatewayApiHeaders() {
    return arcPrivateMainnetHeaders(this._arcPrivateMainnet);
  }
  /**
