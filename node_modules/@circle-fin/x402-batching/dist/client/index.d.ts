import { k as BatchPayload, l as BatchEvmSigner, m as BeforePaymentCreationHook, n as AfterPaymentCreationHook, o as OnPaymentCreationFailureHook, p as OnPaymentResponseHook, q as PaymentResponseContext } from '../hooks-DIYNWsEd.js';
export { r as ARC_PRIVATE_MAINNET_HEADER, G as GATEWAY_AUTH_VALIDITY_WINDOW_SECONDS, s as arcPrivateMainnetHeaders } from '../hooks-DIYNWsEd.js';
 ...
 *
 * Lifecycle hooks (`onBeforePaymentCreation`, `onAfterPaymentCreation`,
 * `onPaymentCreationFailure`, `onPaymentResponse`) mirror the standard SDK's
 * `x402Client` semantics — see https://docs.x402.org/advanced-concepts/lifecycle-hooks.
 *
 * Hooks fire inside `createPaymentPayload` so that this scheme can be used
 * standalone (without `x402Client`). When wrapped by an `x402Client`, register
 * hooks at that level instead — running them in both places would double-fire.
 *
 * @example
 * ```typescript
...
declare class BatchEvmScheme implements SchemeNetworkClient$1 {
     ...
     */
    dispatchPaymentResponse(context: PaymentResponseContext): Promise<{
        recovered: true;
    } | undefined>;
    /**
     ...
     */
    private synthesizePaymentRequired;
}
...
interface GatewayClientConfig {
    ...
    privateKey: Hex;
    /**
     * Custom RPC URL. Optional for most chains (a sensible default is used).
     *
     * Required for `chain: 'arc'`: Arc mainnet has no public RPC until ~2026-06-22,
     * so you must pass a private RPC URL (Alchemy / Quicknode / Circle-provided).
     */
    rpcUrl?: string;
    /**
     * Send `X-ARC-PRIVATE-MAINNET-ENABLED: true` on Gateway API calls.
     * Defaults to `true` when `chain` is `arc` (Arc mainnet internal preview).
     */
