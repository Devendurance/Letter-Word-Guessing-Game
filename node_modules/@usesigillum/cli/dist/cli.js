...
import { createSigillumClient, loadSigillumEnvFiles, } from "./lib/cli-client.js";
...
const DEFAULT_BASE_URL = "https://usesigillum.vercel.app";
async function main() {
    loadSigillumEnvFiles({ fs, path });
    try {
        const args = parseArgs(process.argv.slice(2));
        const diff = readDiffInput(args);
        const client = createSigillumClient({ baseUrl: args.baseUrl, allowDemoConfirm: true });
...
function parseArgs(argv) {
    ...
    if (commandToken !== "inspect" && commandToken !== "quote") {
        throw new CliCommandError("usage", "Usage: sigillum <inspect|quote> <diff-path|--git-diff> [options]");
    }
    const parsed = {
...
function readCurrentGitDiff() {
    try {
        ...
        throw new CliCommandError("input", "Unable to read the current git diff.", error instanceof Error ? error.message : String(error));
    }
}
function maybeSaveReceipt(savePath, result) {
    if (!savePath) {
        return undefined;
    }
...
function renderQuoteOutput(quote, asJson) {
    if (asJson) {
...
function renderInspectOutput(result, asJson, savedReceiptPath) {
    if (asJson) {
        console.log(JSON.stringify({
            quote: result.quote,
...
function exitCodeForRecommendation(recommendation, failOnWarn) {
...
function readEnvFromProcess(...keys) {
...
function isCliErrorShape(error) {
